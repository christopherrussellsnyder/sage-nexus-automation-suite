...
const generateModernTemplate = (data: WebsiteData, businessType: string, templateId: number, baseStyles: string): Template => {
  ...
  const html = generateHTML(data, currentStyle, content);
...
const generateHTML = (data: WebsiteData, currentStyle: any, content: any): string => {
  return `
<!DOCTYPE html>
<html lang="en">
<head>
        ...
        <section class="section" style="background: hsl(var(--secondary) / 0.3);" id="services">
            <div class="container">
                ...
                <div class="grid grid-2">
                    ${content.sections.slice(1, 3).map((section: string, index: number) => `
